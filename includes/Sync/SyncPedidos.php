...
class SyncPedidos {
	...
	public static function sync_batch( \MiIntegracionApi\Core\ApiConnector $api_connector, array $order_ids = array(), array $filters = array(), bool $force_restart = false ) {
		...
		try {
			...
			/* 
			// El código siguiente ya no se usa directamente, queda comentado como referencia
			// Se ha trasladado a process_single_order para mejor mantenibilidad
			foreach ( $filtered_order_ids as $order_id ) {
				$order = wc_get_order( $order_id );
				if ( ! $order instanceof \WC_Order ) {
					++$errors;
					$error_msg = sprintf( __( 'Pedido ID %d no encontrado o inválido (batch).', 'mi-integracion-api' ), $order_id );
					$log[]     = $error_msg;
					\MiIntegracionApi\helpers\Logger::warning( $error_msg, array( 'context' => 'sync-pedidos-batch' ) );
					continue;
				}
				$verial_doc_id = $order->get_meta( '_verial_documento_id' );
				...
				if ( $es_modificable ) {
					...
					if ( is_wp_error( $resp_update ) ) {
						++$errors;
						$error_msg = sprintf( __( 'Error al actualizar pedido ID %1$d en Verial: %2$s', 'mi-integracion-api' ), $order_id, $resp_update->get_error_message() );
						$log[]     = $error_msg;
						\MiIntegracionApi\helpers\Logger::error( $error_msg, array( 'context' => 'sync-pedidos-batch' ) );
						continue;
					} elseif ( isset( $resp_update['InfoError']['Codigo'] ) && intval( $resp_update['InfoError']['Codigo'] ) === 0 ) {
						++$processed;
						$log_msg = sprintf( __( 'Pedido ID %1$d actualizado en Verial (ID: %2$s).', 'mi-integracion-api' ), $order_id, $verial_doc_id );
						$log[]   = $log_msg;
						\MiIntegracionApi\helpers\Logger::info( $log_msg, array( 'context' => 'sync-pedidos-batch' ) );
						continue;
					} else {
						++$errors;
						...
						$error_msg  = sprintf( __( 'Error al actualizar pedido ID %1$d en Verial: %2$s', 'mi-integracion-api' ), $order_id, $error_desc );
						$log[]      = $error_msg;
						\MiIntegracionApi\helpers\Logger::error( $error_msg . ' Respuesta Verial: ' . wp_json_encode( $resp_update ), array( 'context' => 'sync-pedidos-batch' ) );
						continue;
					}
				} else {
					$log_msg = sprintf( __( 'Pedido ID %1$d omitido (batch): ya sincronizado y no modificable en Verial (ID: %2$s).', 'mi-integracion-api' ), $order_id, $verial_doc_id );
					$log[]   = $log_msg;
					\MiIntegracionApi\helpers\Logger::info( $log_msg, array( 'context' => 'sync-pedidos-batch' ) );
					continue;
				}

				// --- Control de duplicados: omitir si ya sincronizado ---
				$verial_doc_id = $order->get_meta( '_verial_documento_id' );
				if ( $verial_doc_id ) {
					++$processed;
					$log_msg = sprintf( __( 'Pedido ID %1$d omitido (batch): ya sincronizado con Verial (ID: %2$s).', 'mi-integracion-api' ), $order_id, $verial_doc_id );
					$log[]   = $log_msg;
					\MiIntegracionApi\helpers\Logger::info( 'Pedido ID ' . $order_id . ' omitido (batch): ya sincronizado con Verial (ID: ' . $verial_doc_id . ')', array( 'context' => 'sync-pedidos-batch' ) );
					continue;
				}
				// --- Sincronización incremental: omitir si no hay cambios desde la última sync (si se provee filtro 'date_modified') ---
				if ( isset( $filters['date_modified'] ) ) {
					$order_modified = $order->get_date_modified() ? $order->get_date_modified()->date( 'Y-m-d H:i:s' ) : null;
					if ( $order_modified && strtotime( $order_modified ) < strtotime( $filters['date_modified'] ) ) {
						$log[] = sprintf( __( 'Pedido ID %1$d omitido (batch): no modificado desde %2$s.', 'mi-integracion-api' ), $order_id, $filters['date_modified'] );
						continue;
					}
				}
				...
				if ( ! $cliente_info_presente ) {
					++$errors;
					$error_msg = sprintf( __( 'Pedido ID %d omitido (batch): Información del cliente insuficiente para Verial.', 'mi-integracion-api' ), $order->get_id() );
					$log[]     = $error_msg;
					\MiIntegracionApi\helpers\Logger::warning( $error_msg, array( 'context' => 'sync-pedidos-batch' ) );
					continue;
				}
				if ( empty( $payload_documento_verial['Contenido'] ) || ! is_array( $payload_documento_verial['Contenido'] ) ) {
					++$errors;
					$error_msg = sprintf( __( 'Pedido ID %d omitido (batch): No se generaron líneas de contenido válidas para Verial.', 'mi-integracion-api' ), $order->get_id() );
					$log[]     = $error_msg;
					\MiIntegracionApi\helpers\Logger::warning( $error_msg, array( 'context' => 'sync-pedidos-batch' ) );
					continue;
				}
				...
				if ( is_wp_error( $response_verial ) ) {
					// Manejo del error
					return $response_verial;
				} elseif ( isset( $response_verial['InfoError']['Codigo'] ) && intval( $response_verial['InfoError']['Codigo'] ) === 0 ) {
					++$processed;
					$log_msg = sprintf( __( 'Pedido ID %1$d sincronizado (batch). ID Documento Verial: %2$s, Número Verial: %3$s', 'mi-integracion-api' ), $order->get_id(), ( $response_verial['Id'] ?? 'N/A' ), ( $response_verial['Numero'] ?? 'N/A' ) );
					$log[]   = $log_msg;
					if ( isset( $response_verial['Id'] ) ) {
						$order->update_meta_data( '_verial_documento_id', intval( $response_verial['Id'] ) );
					}
					...
					++$errors;
					$error_desc = isset( $response_verial['InfoError']['Descripcion'] ) ? $response_verial['InfoError']['Descripcion'] : __( 'Error desconocido de Verial', 'mi-integracion-api' );
					$error_msg  = sprintf( __( 'Error al sincronizar pedido ID %1$d (batch) con Verial: %2$s', 'mi-integracion-api' ), $order->get_id(), $error_desc );
					$log[]      = $error_msg;
					\MiIntegracionApi\helpers\Logger::error( $error_msg . ' Respuesta Verial: ' . wp_json_encode( $response_verial ), array( 'context' => 'sync-pedidos-batch' ) );
				}
			}
			...
			\MiIntegracionApi\helpers\Logger::info( $final_message, array( 'context' => 'sync-pedidos-batch' ) );
			...
			*/ // Fin del código comentado que ya no se usa
